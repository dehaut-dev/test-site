...
!function (a, b) {
  ...
  function j(a) {
    var b = "Webkit Moz ms O".split(" "),
        c = document.documentElement.style;if (void 0 !== c[a]) return a;a = a.charAt(0).toUpperCase() + a.substr(1);for (var d = 0; d < b.length; d++) {
    ...
    this.options = this.applyDefaults(b || {}), this.bindMethods(), this.wrapper = this.getWrapperElement(_a), this.wrapper && (this.handle = this.getHandleElement(this.wrapper, this.options.handleClass), this.handle && (this.init(), this.bindEventListeners()));
  };a.prototype = { defaults: { disabled: !1, horizontal: !0, vertical: !1, slide: !0, steps: 0, snap: !1, loose: !1, speed: .1, xPrecision: 0, yPrecision: 0, handleClass: "handle", css3: !0, activeClass: "active", tapping: !0 }, init: function init() {
      this.options.css3 && k(this.handle), this.value = { prev: [-1, -1], current: [this.options.x || 0, this.options.y || 0], target: [this.options.x || 0, this.options.y || 0] }, this.offset = { wrapper: [0, 0], mouse: [0, 0], prev: [-999999, -999999], current: [0, 0], target: [0, 0] }, this.change = [0, 0], this.stepRatios = this.calculateStepRatios(), this.activity = !1, this.dragging = !1, this.tapping = !1, this.reflow(), this.options.disabled && this.disable();
    }, applyDefaults: function applyDefaults(a) {
      for (var b in this.defaults) {
        a.hasOwnProperty(b) || (a[b] = this.defaults[b]);
      }return a;
    }, getWrapperElement: function getWrapperElement(a) {
      return "string" == typeof a ? document.getElementById(a) : a;
    }, getHandleElement: function getHandleElement(a, b) {
      var c, d, e;if (a.getElementsByClassName) {
        if (c = a.getElementsByClassName(b), c.length > 0) return c[0];
      } else for (d = new RegExp("(^|\\s)" + b + "(\\s|$)"), c = a.getElementsByTagName("*"), e = 0; e < c.length; e++) {
        if (d.test(c[e].className)) return c[e];
      }
    }, calculateStepRatios: function calculateStepRatios() {
      var a = [];if (this.options.steps >= 1) for (var b = 0; b <= this.options.steps - 1; b++) {
        a[b] = this.options.steps > 1 ? b / (this.options.steps - 1) : 0;
      }return a;
    }, setWrapperOffset: function setWrapperOffset() {
      this.offset.wrapper = h.get(this.wrapper);
    }, calculateBounds: function calculateBounds() {
      var a = { top: this.options.top || 0, bottom: -(this.options.bottom || 0) + this.wrapper.offsetHeight, left: this.options.left || 0, right: -(this.options.right || 0) + this.wrapper.offsetWidth };return a.availWidth = a.right - a.left - this.handle.offsetWidth, a.availHeight = a.bottom - a.top - this.handle.offsetHeight, a;
    }, calculateValuePrecision: function calculateValuePrecision() {
      var a = this.options.xPrecision || Math.abs(this.bounds.availWidth),
          b = this.options.yPrecision || Math.abs(this.bounds.availHeight);return [a ? 1 / a : 0, b ? 1 / b : 0];
    ...
    }, onHandleMouseDown: function onHandleMouseDown(a) {
      g.refresh(a), e(a), f(a), this.activity = !1, this.startDrag();
    }, onHandleTouchStart: function onHandleTouchStart(a) {
    ...
    }, onWrapperTouchMove: function onWrapperTouchMove(a) {
    ...
    }, setStep: function setStep(a, b, c) {
      ...
      if (!this.disabled && this.dragging) {
        this.dragging = !1;var a = this.groupClone(this.value.current);if (this.options.slide) {
          var b = this.change;a[0] += 4 * b[0], a[1] += 4 * b[1];
      ...
      var a = this.value.current;this.options.snap && this.options.steps > 1 && (a = this.getClosestSteps(a)), this.groupCompare(a, this.value.prev) || ("function" == typeof this.options.animationCallback && this.options.animationCallback.call(this, a[0], a[1]), this.groupCopy(this.value.prev, a));
    ...
    }, callDragStopCallback: function callDragStopCallback() {
    ...
    }, animateWithRequestAnimationFrame: function animateWithRequestAnimationFrame(a) {
    ...
    }, animate: function animate(a, b) {
      if (!a || this.dragging) {
        if (this.dragging) {
          var c = this.groupClone(this.value.target),
              d = [g.x - this.offset.wrapper[0] - this.offset.mouse[0], g.y - this.offset.wrapper[1] - this.offset.mouse[1]];this.setTargetValueByOffset(d, this.options.loose), this.change = [this.value.target[0] - c[0], this.value.target[1] - c[1]];
      ...
      var a = [this.value.target[0] - this.value.current[0], this.value.target[1] - this.value.current[1]];return a[0] || a[1] ? (Math.abs(a[0]) > this.valuePrecision[0] || Math.abs(a[1]) > this.valuePrecision[1] ? (this.value.current[0] += a[0] * Math.min(this.options.speed * this.timeOffset / 25, 1), this.value.current[1] += a[1] * Math.min(this.options.speed * this.timeOffset / 25, 1)) : this.groupCopy(this.value.current, this.value.target), !0) : !1;
      ...
      var a = "";return this.options.css3 && i.transform ? (this.options.horizontal && (a += "translateX(" + this.offset.current[0] + "px)"), this.options.vertical && (a += " translateY(" + this.offset.current[1] + "px)"), this.handle.style[i.transform] = a, void 0) : (this.options.horizontal && (this.handle.style.left = this.offset.current[0] + "px"), this.options.vertical && (this.handle.style.top = this.offset.current[1] + "px"), void 0);
    }, setTargetValue: function setTargetValue(a, b) {
      var c = b ? this.getLooseValue(a) : this.getProperValue(a);this.groupCopy(this.value.target, c), this.offset.target = this.getOffsetsByRatios(c), this.callTargetCallback();
    }, setTargetValueByOffset: function setTargetValueByOffset(a, b) {
      var c = this.getRatiosByOffsets(a),
          d = b ? this.getLooseValue(c) : this.getProperValue(c);this.groupCopy(this.value.target, d), this.offset.target = this.getOffsetsByRatios(d);
    }, getLooseValue: function getLooseValue(a) {
      var b = this.getProperValue(a);return [b[0] + (a[0] - b[0]) / 4, b[1] + (a[1] - b[1]) / 4];
    }, getProperValue: function getProperValue(a) {
      var b = this.groupClone(a);return b[0] = Math.max(b[0], 0), b[1] = Math.max(b[1], 0), b[0] = Math.min(b[0], 1), b[1] = Math.min(b[1], 1), (!this.dragging && !this.tapping || this.options.snap) && this.options.steps > 1 && (b = this.getClosestSteps(b)), b;
    }, getRatiosByOffsets: function getRatiosByOffsets(a) {
    ...
    }, getRatioByOffset: function getRatioByOffset(a, b, c) {
      return b ? (a - c) / b : 0;
    }, getOffsetsByRatios: function getOffsetsByRatios(a) {
    ...
    }, getOffsetByRatio: function getOffsetByRatio(a, b, c) {
      return Math.round(a * b) + c;
    }, getStepNumber: function getStepNumber(a) {
      return this.getClosestStep(a) * (this.options.steps - 1) + 1;
    }, getClosestSteps: function getClosestSteps(a) {
      return [this.getClosestStep(a[0]), this.getClosestStep(a[1])];
    }, getClosestStep: function getClosestStep(a) {
      for (var b = 0, c = 1, d = 0; d <= this.options.steps - 1; d++) {
        Math.abs(this.stepRatios[d] - a) < c && (c = Math.abs(this.stepRatios[d] - a), b = d);
    ...
    a.addEventListener ? a.addEventListener(b, _c, !1) : a.attachEvent && a.attachEvent("on" + b, _c);
  }, d = function d(a, b, c) {
  ...
  }, g = { x: 0, y: 0, xDiff: 0, yDiff: 0, refresh: function refresh(a) {
      a || (a = window.event), "mousemove" == a.type ? this.set(a) : a.touches && this.set(a.touches[0]);
    }, set: function set(a) {
      var b = this.x,
          c = this.y;a.clientX || a.clientY ? (this.x = a.clientX, this.y = a.clientY) : (a.pageX || a.pageY) && (this.x = a.pageX - document.body.scrollLeft - document.documentElement.scrollLeft, this.y = a.pageY - document.body.scrollTop - document.documentElement.scrollTop), this.xDiff = Math.abs(this.x - b), this.yDiff = Math.abs(this.y - c);
    } }, h = { get: function get(a) {
      var b = { left: 0, top: 0 };return void 0 !== a.getBoundingClientRect && (b = a.getBoundingClientRect()), [b.left, b.top];
    } }, i = { transform: j("transform"), perspective: j("perspective"), backfaceVisibility: j("backfaceVisibility") }, l = ["webkit", "moz"], m = window.requestAnimationFrame, n = window.cancelAnimationFrame, o = 0; o < l.length && !m; ++o) {
    m = window[l[o] + "RequestAnimationFrame"], n = window[l[o] + "CancelAnimationFrame"] || window[l[o] + "CancelRequestAnimationFrame"];
  }return m || (m = function m(a) {
